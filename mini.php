...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['rent_now'])) {
    ...
    // Redirect back to mini.php to avoid form resubmission
...
// Fetch mini dresses from the database
$stmt = $pdo->prepare("SELECT * FROM dresses WHERE category = 'Mini Dress'");
$stmt->execute();
...
<head>
    ...
    <title>Mini Dresses - Fancy Dress Rental</title>
...
<body>
    ...
    <div class="container">
            ...
            <ul>
                <?php foreach ($categories as $category): ?>
                    ...
                    <?php elseif ($category === 'Dresses'): ?>
                        <li><a href="dress.php"><?php echo htmlspecialchars($category); ?></a></li>
        ...
        <!-- Content Grid -->
        <main class="content">
            <h2 class="category-title">MINI DRESSES</h2>
            <div class="grid">
                <?php if (count($dresses) > 0): ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <p>No mini dresses found in this category.</p>
                <?php endif; ?>
            </div>
        </main>
    </div>
